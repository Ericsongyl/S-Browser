...
package com.nicksong.falcon.providers;
...
import com.nicksong.falcon.model.BookmarkHistoryItem;
import com.nicksong.falcon.model.FolderItem;
import com.nicksong.falcon.utils.Constants;

public class BookmarksWrapper {

	public static String[] HISTORY_BOOKMARKS_PROJECTION = new String[] { BookmarksProvider.Columns._ID, BookmarksProvider.Columns.TITLE,
			BookmarksProvider.Columns.URL, BookmarksProvider.Columns.VISITS, BookmarksProvider.Columns.CREATION_DATE, BookmarksProvider.Columns.VISITED_DATE,
			BookmarksProvider.Columns.BOOKMARK, BookmarksProvider.Columns.IS_FOLDER, BookmarksProvider.Columns.PARENT_FOLDER_ID,
			BookmarksProvider.Columns.FAVICON, BookmarksProvider.Columns.THUMBNAIL };
	...
	public static CursorLoader getCursorLoaderForStartPage(Context context, int limit) {
		...
		String orderClause = BookmarksProvider.Columns.VISITS + " DESC, " + BookmarksProvider.Columns.VISITED_DATE + " DESC LIMIT " + Integer.toString(limit);
	...
	public static CursorLoader getCursorLoaderForBookmarks(Context context, long parentFolderId) {
		...
		String whereClause = BookmarksProvider.Columns.PARENT_FOLDER_ID + " = " + parentFolderId + " AND (" + BookmarksProvider.Columns.BOOKMARK + " = 1 OR "
				+ BookmarksProvider.Columns.IS_FOLDER + " = 1)";

		String orderClause;
		switch (sortMode) {
		case 0:
			orderClause = BookmarksProvider.Columns.IS_FOLDER + " DESC, " + BookmarksProvider.Columns.VISITS + " DESC, " + BookmarksProvider.Columns.TITLE
					+ " COLLATE NOCASE";
			break;

		case 1:
			orderClause = BookmarksProvider.Columns.IS_FOLDER + " DESC, " + BookmarksProvider.Columns.TITLE + " COLLATE NOCASE, "
					+ BookmarksProvider.Columns.VISITS + " DESC";
			break;

		case 2:
			orderClause = BookmarksProvider.Columns.IS_FOLDER + " DESC, " + BookmarksProvider.Columns.VISITED_DATE + " DESC, "
					+ BookmarksProvider.Columns.TITLE + " COLLATE NOCASE";
			break;
	...
	public static void clearHistoryAndOrBookmarks(ContentResolver contentResolver, boolean clearHistory, boolean clearBookmarks) {
		...
		if (clearHistory && clearBookmarks) {
			...
			whereClause = "((" + BookmarksProvider.Columns.BOOKMARK + " = 0) OR (" + BookmarksProvider.Columns.BOOKMARK + " IS NULL)) AND ("
	...
	/**
	 * Modify a bookmark/history record. If an id is provided, it look for it and update its values. If not, values will be inserted. If no id is provided, it look for a record with the given url. It found, its values are updated. If not, values will be inserted.
	 * 
	 * @param contentResolver
	 *            The content resolver.
	 * @param id
	...
	public static void deleteFolder(ContentResolver contentResolver, long id) {
		...
		BookmarksProvider provider = (BookmarksProvider) contentResolver.acquireContentProviderClient(BookmarksProvider.BOOKMARKS_URI)
	...
	public static void updateHistory(ContentResolver contentResolver, String title, String url, String originalUrl) {
		String[] colums = new String[] { BookmarksProvider.Columns._ID, BookmarksProvider.Columns.URL, BookmarksProvider.Columns.BOOKMARK,
	...
	private static final String TRUNCATE_HISTORY_DELETE_WHERE_PATTERN = "(" + BookmarksProvider.Columns.BOOKMARK + " = 0 OR "
			+ BookmarksProvider.Columns.BOOKMARK + " IS NULL) AND " + BookmarksProvider.Columns.VISITED_DATE + " < %s";
	private static final String TRUNCATE_HISTORY_UPDATE_WHERE_PATTERN = BookmarksProvider.Columns.BOOKMARK + " = 1  AND "
			+ BookmarksProvider.Columns.VISITED_DATE + " < %s";

	/**
	 * Remove from history values prior to now minus the number of days defined in preferences. Only delete history items. For bookmarks, reset their visited value date and visits count.
	 * 
	 * @param contentResolver
	 *            The content resolver.
	 */
	...
	public static void updateFavicon(ContentResolver contentResolver, String url, String originalUrl, Bitmap favicon) {
		if ((url != null) && (favicon != null) && (contentResolver != null)) {
			String whereClause;

			if ((originalUrl != null) && !url.equals(originalUrl)) {
	...
	private static final String BOOKMARK_BY_URL_WHERE_PATTERN_1 = "(" + BookmarksProvider.Columns.URL + " = %s OR " + BookmarksProvider.Columns.URL
			+ " = %s) AND " + BookmarksProvider.Columns.BOOKMARK + " = 1";
	...
	public static void updateThumbnail(ContentResolver contentResolver, String url, String originalUrl, Bitmap thumbnail) {
		if ((url != null) && (thumbnail != null) && (contentResolver != null)) {
			String whereClause;
	...
	public static boolean urlHasBookmark(ContentResolver contentResolver, String url, String originalUrl) {
		if ((url != null) && (contentResolver != null)) {
			String whereClause;
	...
	private static final String SUGGESTIONS_ORDER = BookmarksProvider.Columns.VISITED_DATE + " DESC, " + BookmarksProvider.Columns.BOOKMARK + " DESC, "
			+ BookmarksProvider.Columns.TITLE + " ASC";

	/**
	 * Get a cursor for suggestions, given a search pattern. Search on history and bookmarks, on title and url.
	 * 
	 * @param contentResolver
	 *            The content resolver.
	 * @param pattern
	...
	public static Cursor getUrlSuggestions(ContentResolver contentResolver, String pattern) {
		if ((pattern != null) && (pattern.length() > 0)) {
			...
			return contentResolver.query(BookmarksProvider.BOOKMARKS_URI, HISTORY_BOOKMARKS_PROJECTION, whereClause, null, SUGGESTIONS_ORDER);
		}
