...
import com.nicksong.falcon.providers.BookmarksWrapper;
import com.nicksong.falcon.tasks.HistoryBookmarksExportTask;
import com.nicksong.falcon.tasks.HistoryBookmarksImportTask;
import com.nicksong.falcon.ui.fragments.BookmarksFragment;
import com.nicksong.falcon.ui.fragments.HistoryFragment;
import com.nicksong.falcon.ui.managers.UIManager;
import com.nicksong.falcon.ui.preferences.IHistoryBookmaksExportListener;
import com.nicksong.falcon.ui.preferences.IHistoryBookmaksImportListener;
import com.nicksong.falcon.ui.tabs.GenericTabListener;
import com.nicksong.falcon.utils.ApplicationUtils;
import com.nicksong.falcon.utils.Constants;
import com.nicksong.falcon.utils.IOUtils;
...
public class BookmarksActivity extends Activity implements IHistoryBookmaksExportListener, IHistoryBookmaksImportListener {
	...
	private static final AtomicReference<AsyncTask<String, Integer, String>> mImportSyncThread = new AtomicReference<AsyncTask<String, Integer, String>>();

	private static final AtomicReference<AsyncTask<Cursor, Integer, String>> mExportSyncThread = new AtomicReference<AsyncTask<Cursor, Integer, String>>();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if ((savedInstanceState != null) && (savedInstanceState.containsKey(EXTRA_SELECTED_TAB_INDEX))) {
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		List<AddonMenuItem> contributedMenuItems = Controller.getInstance().getAddonManager()
				.getContributedHistoryBookmarksMenuItems(mUIManager.getCurrentWebView());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		default:
			if (Controller.getInstance().getAddonManager()
					.onContributedHistoryBookmarksMenuItemSelected(this, item.getItemId(), mUIManager.getCurrentWebView())) {
				return true;
			} else {
	...
	public void onExportDone(String message) {
		...
		if (message != null) {
			ApplicationUtils.showErrorDialog(this, getString(R.string.HistoryBookmarksExportErrorTitle),
	...
	public void onImportDone(String message) {
		...
		if (message != null) {
			ApplicationUtils.showErrorDialog(this, getString(R.string.HistoryBookmarksImportErrorTitle),
	...
	private void importHistoryBookmarks() {
		...
		builder.setSingleChoiceItems(choices, 0, new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				mProgress = ProgressDialog.show(BookmarksActivity.this, getString(R.string.HistoryBookmarksImportTitle),
						getString(R.string.HistoryBookmarksImportInitialMessage), true, false);
	...
	private void exportHistoryBookmarks() {
		...
		mProgress = ProgressDialog.show(this, getString(R.string.HistoryBookmarksExportTitle), getString(R.string.HistoryBookmarksExportInitialMessage), true,
				false);
	...
	private void changeSortMode() {
		...
		builder.setSingleChoiceItems(new String[] { getResources().getString(R.string.MostUsedSortMode), getResources().getString(R.string.AlphaSortMode),
				getResources().getString(R.string.RecentSortMode) }, currentSort, new OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
