...
package com.nicksong.falcon.ui.dialogs;
...
public class DownloadConfirmDialog {
	...
	public DownloadConfirmDialog(Context context) {
		...
		mBuilder = new AlertDialog.Builder(mContext).setView(mView).setTitle(R.string.DownloadDialogTitle);
	}
	...
	public void show() {
		mBuilder.setPositiveButton(mContext.getResources().getText(R.string.Download), new OnClickListener() {
			@Override
		...
		mBuilder.setNegativeButton(mContext.getResources().getText(R.string.Cancel), new OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				sendDenyCallback();
			}
