...
public class ScrollerView extends FrameLayout {
	...
	public int getMaxScrollAmount() {
		return (int) (MAX_SCROLL_FACTOR * (mHorizontal ? (getRight() - getLeft()) : (getBottom() - getTop())));
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			final float y = mHorizontal ? ev.getX(pointerIndex) : ev.getY(pointerIndex);
			...
			if (yDiff > mTouchSlop) {
				...
				final float ocoord = mHorizontal ? ev.getY(pointerIndex) : ev.getX(pointerIndex);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (isOrthoMove(x - mDownCoords.x, y - mDownCoords.y)) {
				...
				// if (isOrthoMove(x - mDownCoords.x, y - mDownCoords.y)) {
				onOrthoDrag(mDownView, mHorizontal ? y - mDownCoords.y : x - mDownCoords.x);
				// }
				...
				if (mHorizontal) {
					if (overScrollBy(deltaY, 0, getScrollX(), 0, range, 0, mOverscrollDistance, 0, true)) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (isOrthoMove(vtracker.getXVelocity(mActivePointerId), vtracker.getYVelocity(mActivePointerId))
					&& mMinimumVelocity < Math.abs((mHorizontal ? vtracker.getYVelocity() : vtracker.getXVelocity()))) {
				onOrthoFling(mDownView, mHorizontal ? vtracker.getYVelocity() : vtracker.getXVelocity());
				break;
			}
			if (mIsOrthoDragged) {
				...
				int initialVelocity = mHorizontal ? (int) velocityTracker.getXVelocity(mActivePointerId) : (int) velocityTracker.getYVelocity(mActivePointerId);
		...
		case MotionEvent.ACTION_POINTER_UP:
			onSecondaryPointerUp(ev);
			mLastMotionY = mHorizontal ? ev.getX(ev.findPointerIndex(mActivePointerId)) : ev.getY(ev.findPointerIndex(mActivePointerId));
			break;
		}
		return true;
	}
	...
	private void onSecondaryPointerUp(MotionEvent ev) {
		...
		if (pointerId == mActivePointerId) {
			...
			mLastOrthoCoord = mHorizontal ? ev.getY(newPointerIndex) : ev.getX(newPointerIndex);
		}
	}
	...
	private int getScrollRange() {
		...
		if (getChildCount() > 0) {
			View child = getChildAt(0);
			if (mHorizontal) {
				scrollRange = Math.max(0, child.getWidth() - (getWidth() - getPaddingRight() - getPaddingLeft()));
			} else {
	 ...
	 * 
	 * @param topFocus
	 *            look for a candidate is the one at the top of the bounds if topFocus is true, or at the bottom of the bounds if topFocus is false
	 * @param top
		...
		for (int i = 0; i < count; i++) {
			...
			if (top < viewBottom && viewTop < bottom) {
				...
				if (focusCandidate == null) {
					...
					final boolean viewIsCloserToBoundary = (topFocus && viewTop < ctop) || (!topFocus && viewBottom > cbot);
	 ...
	 * 
	 * @param direction
	 *            the scroll direction: {@link android.view.View#FOCUS_UP} to go one page up or {@link android.view.View#FOCUS_DOWN} to go one page down
	 ...
	 * 
	 * @param direction
	 *            the scroll direction: {@link android.view.View#FOCUS_UP} to go the top of the view or {@link android.view.View#FOCUS_DOWN} to go the bottom
	 * @return true if the key event is consumed by this method, false otherwise
	 */
	 ...
	 * 
	 * @param direction
	 *            the scroll direction: {@link android.view.View#FOCUS_UP} to go upward, {@link android.view.View#FOCUS_DOWN} to downward
	 * @param top
	...
	private boolean isOrthoMove(float moveX, float moveY) {
		return mHorizontal && Math.abs(moveY) > Math.abs(moveX) || !mHorizontal && Math.abs(moveX) > Math.abs(moveY);
	}
	...
	private boolean isWithinDeltaOfScreen(View descendant, int delta, int height) {
		...
		if (mHorizontal) {
			return (mTempRect.right + delta) >= getScrollX() && (mTempRect.left - delta) <= (getScrollX() + height);
		} else {
	...
	protected void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
		...
		int childHeightMeasureSpec;
		if (mHorizontal) {
			childHeightMeasureSpec = getChildMeasureSpec(parentHeightMeasureSpec, getPaddingTop() + getPaddingBottom() + lp.topMargin + lp.bottomMargin
					+ heightUsed, lp.height);
			childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(lp.leftMargin + lp.rightMargin, MeasureSpec.UNSPECIFIED);
		} else {
			childWidthMeasureSpec = getChildMeasureSpec(parentWidthMeasureSpec, getPaddingLeft() + getPaddingRight() + lp.leftMargin + lp.rightMargin
					+ widthUsed, lp.width);
	...
	public void fling(int velocityY) {
		if (getChildCount() > 0) {
			if (mHorizontal) {
				...
				mScroller.fling(getScrollX(), getScrollY(), velocityY, 0, 0, Math.max(0, right - width), 0, 0, width / 2, 0);
			} else {
