...
public abstract class BaseUIManager implements UIManager {// , WebViewFragmentListener {
	...
	public void addTab(boolean loadHomePage, boolean privateBrowsing, boolean openByParent) {
		if (loadHomePage) {
			addTab(PreferenceManager.getDefaultSharedPreferences(mActivity).getString(Constants.PREFERENCE_HOME_PAGE, Constants.URL_ABOUT_START), false,
					privateBrowsing, openByParent);
		} else {
	...
	public void loadHomePage(UUID tabId, boolean loadInCurrentTabIfNotFound) {
		mHomePageLoading = true;
		loadUrl(tabId, PreferenceManager.getDefaultSharedPreferences(mActivity).getString(Constants.PREFERENCE_HOME_PAGE, Constants.URL_ABOUT_START),
			...
			if (Intent.ACTION_VIEW.equals(intent.getAction()) || Intent.ACTION_MAIN.equals(intent.getAction())) {
				...
				if (!TextUtils.isEmpty(url)) {
					if (!isCurrentTabReusable()) {
						addTab(url,
								false,
								PreferenceManager.getDefaultSharedPreferences(this.getMainActivity()).getBoolean(Constants.PREFERENCE_INCOGNITO_BY_DEFAULT,
										false), false);
					} else {
						loadUrl(url);
					}
					...
					case SealBrowserActivity.CONTEXT_MENU_DOWNLOAD:
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT, -1)) {
							...
							try {
								...
								Toast.makeText(mActivity, String.format(mActivity.getString(R.string.DownloadStart), item.getFileName()), Toast.LENGTH_SHORT)
										.show();
					...
					default:
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT, -1)) {
							...
							Controller
									.getInstance()
									.getAddonManager()
									.onContributedContextLinkMenuItemSelected(mActivity, actionId, intent.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT, -1),
											intent.getStringExtra(Constants.EXTRA_URL), getCurrentWebView());
						}
						break;
					}
				}
		...
		view.postDelayed(new Runnable() {
			...
			public void run() {
				if (BookmarksWrapper.urlHasBookmark(mActivity.getContentResolver(), url, view.getOriginalUrl())) {
					...
					new ThumbnailSaver(mActivity.getContentResolver(), url, view.getOriginalUrl(), p, ApplicationUtils
							.getBookmarksThumbnailsDimensions(mActivity)).execute();
				}
			}
		}, 2000);
	}
	...
	public void saveTabs() {
		...
		for (BaseWebViewFragment f : getTabsFragments()) {
			if (!f.isStartPageShown() && !f.isWebViewOnUrl(userStartPage) && !f.isPrivateBrowsingEnabled()) {
	...
	protected boolean isHomePageStartPage() {
		return Constants.URL_ABOUT_START.equals(PreferenceManager.getDefaultSharedPreferences(mActivity).getString(Constants.PREFERENCE_HOME_PAGE,
				...
				case FOCUS_NODE_HREF:
					...
					default:
						Controller.getInstance().getAddonManager()
								.onContributedContextLinkMenuItemSelected(mActivity, msg.arg1, HitTestResult.SRC_IMAGE_ANCHOR_TYPE, url, getCurrentWebView());
						break;
					}
					break;
				default:
	...
	private void requestHrefNode(int action, boolean incognito) {
		...
		if (webView != null) {
			...
			final Message msg = mHandler.obtainMessage(FOCUS_NODE_HREF, action, incognito ? 1 : 0, hrefMap);
	...
	/**
	 * Check if the current tab can be reused to display an intent request. A tab is reusable if it is on the user-defined start page.
	 * 
