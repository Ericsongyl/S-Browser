...
public class UrlUtils {
	...
	/**
	 * Check if a string is an url. For now, just consider that if a string contains a dot, it is an url.
	 * 
	 * @param url
	 *            The url to check.
	...
	public static boolean isUrl(String url) {
		return url.contains(".") || url.equals(Constants.URL_ABOUT_BLANK) || url.equals(Constants.URL_ABOUT_START);
	}

	public static String getRawSearchUrl(Context context) {
		String currentSearchUrl = PreferenceManager.getDefaultSharedPreferences(context).getString(Constants.PREFERENCE_SEARCH_URL,
	...
	public static String checkUrl(String url) {
		if ((url != null) && (url.length() > 0)) {
			...
			if ((!url.startsWith("http://")) && (!url.startsWith("https://")) && (!url.startsWith("file://")) && (!url.startsWith(Constants.URL_ABOUT_BLANK))
